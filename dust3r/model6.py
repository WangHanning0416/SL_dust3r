...
from .utils.modalities import gen_sparse_depth
from .heads import head_factory
from dust3r.patch_embed import get_patch_embed,PatchEmbed_Mlp
from dust3r.block import Mlp, BlockInject, DecoderBlockInject
...
def save_attn_map(attn_map1, attn_map2, save_dir, layer_idx):
    os.makedirs(os.path.join(save_dir, f"layer_{layer_idx}"), exist_ok=True)
    save_path_f1 = os.path.join(save_dir, f"layer_{layer_idx}/img1_to_img2_attn.npy")
    save_path_f2 = os.path.join(save_dir, f"layer_{layer_idx}/img2_to_img1_attn.npy")
...
class AsymmetricCroCo3DStereo (
    ...
    def _decoder(self, f1, pos1, f2, pos2, relpose1=None, relpose2=None):
        ...
        if relpose1 is not None: # shape = (B, 4, 4)
            if isinstance(relpose1, np.ndarray):
        ...
        if relpose2 is not None:
            if isinstance(relpose2, np.ndarray):
