...
parser.add_argument("--test_name", type=str, required=False, help='')
...
def eval_pointcloud(
    ...
    print(f"eval_pointcloud 接收的 GT 点数量: {len(pointcloud_tgt)}")  # 应接近49980
...
def calcu_pcd_fscore(pcd_rec, pcd_gt, align=True, scale=1, vis_dir=None, voxel_size=0.01):
    ...
    print(f"Reconstructed point num: {len(rec_pointcloud)}, GT point num: {len(gt_pointcloud)}")
...
def calcu_pair_loss(align_gt_pcd, align_pred_pcd,
    ...
    print(f"After alignment, pred pts num: {len(aligned_eval_pred_pcd.vertices)}, gt pts num: {len(gt_pcd.vertices)}")
    ...
    align_flag = True
    print(res2["completeness"], res2["accuracy"])
    if res2["completeness"] > 15 or res2["accuracy"] > 15:
        align_flag = False
...
if __name__ == "__main__":
    ...
    root_dir = "/data3/hanning/dust3r/result"
    for scene_name in scenes:
        npy_dir = os.path.join(root_dir, scene_name, "npy")
        # 加载scene下的点云数据（list，含50组pair的重建结果）
        ...
        save_dir = os.path.join(root_dir, scene_name, "eval")
        os.makedirs(save_dir, exist_ok=True)

        pred_pcd = np.load(pred_path).astype(np.float32) 
        gt_pcd = np.load(gt_path).astype(np.float32)      
        valid_pcd = np.load(valid_path).astype(bool)
        
        all_metrics = []
        for i in range(len(pred_pcd)):
            pred = pred_pcd[i].reshape(-1, 3)
            gt = gt_pcd[i].reshape(-1, 3)
            valid = valid_pcd[i].reshape(-1)

            filtered_gt_pts = gt[valid]
            filtered_pred_pts = pred[valid]
            print(filtered_gt_pts.shape, filtered_pred_pts.shape)
            metric_dict, align_succ = calcu_pair_loss(filtered_gt_pts, 
                                                    filtered_pred_pts,
                                                    c2w=None,
                                                    vis_dir=None,  # For space and time savings, no per-frame visualization
                                                    voxel_size=0.0001)

            if align_succ:
                all_metrics.append(metric_dict)
            else:
                print("Frame evaluation failed. Please check your data or threshold settings.")
        ...
        metric_str = "Acc:{:.3f}, Comp:{:.3f}, f-score20:{:.3f}, f-score15:{:.3f}, f-score10:{:.3f}".format(
            avg_metrics["accuracy"], avg_metrics["completeness"], avg_metrics["f-score-20"], 
            avg_metrics["f-score-15"], avg_metrics["f-score"])
        
        # Save the results to a JSON file
        res_dict = {}
        res_dict['metric'] = metric_str
        res_dict['num_frames_evaluated'] = len(all_metrics)
        print(res_dict)
        
        save_name = f"recon_metric_local.json"
